...
class DataBase:
    ...
    def add_new_student(self, info):
        _student = tuple(info[0].split(','))
        _parent = tuple(info[1].split(','))
        # id_ = info[2]
        # print(type(_student))
        self.insert('human', f"'{_student[0]}', '{_student[1]}', '{_student[2]}', '{_student[3]}'",
                    'id, first_name, second_name, birth_date, phone')
        # self.insert('human', f"'{_parent[0]}', '{_parent[1]}', '{_parent[2]}', '{_parent[3]}'",
        #             'id, first_name, second_name, phone, birth_date')
    ...
    with open(file_name) as fl:
        ...
        if file_name == 'group_.txt':
            for group_line in fl:
                ...
                self.insert(table_name, f"'{group_split[0]}', {group_split[1]}, {group_split[2]},"
                                        f""f" '{group_split[3]}', {group_split[4]}, {group_split[5]}")
            ...
            for human_line in fl:
                ...
                try:
                    if human_line[2] == 'None':
                        self.insert(table_name, f"'{human_line[0]}', '{human_line[1]}', "
                                                f"'{human_line[2]}', "f"'{human_line[3]}'")
